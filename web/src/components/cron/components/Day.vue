      ...
      <n-grid x-gap="20" y-gap="20" :cols="1">
        ...
        <n-gi>
          <n-flex align="center" :wrap="false">
            <n-radio :value="3" />
            周期从
            <n-input-number class="nin" v-model:value="cycle01" :min="1" :max="30" clearable
              @update:value="radioChange(3)" @focus="radioChange(3)" />
            到
            <n-input-number class="nin" v-model:value="cycle02" :min="cycle01 ? cycle01 + 1 : 2" :max="31" clearable
              @update:value="radioChange(3)" @focus="radioChange(3)" />日
          </n-flex>
        </n-gi>
        <n-gi>
          <n-flex align="center" :wrap="false">
            <n-radio :value="4" />
            周期从
            <n-input-number class="nin" v-model:value="average01" :min="1" :max="30" clearable
              @update:value="radioChange(4)" @focus="radioChange(4)" />
            号开始，每
            <n-input-number class="nin" v-model:value="average02" :min="1" :max="31 - average01 || 1" clearable
              @update:value="radioChange(4)" @focus="radioChange(4)" />日执行一次
          </n-flex>
        </n-gi>
        <n-gi>
          <n-flex align="center">
            <n-radio :value="5" /> 每个月
            <n-input-number class="nin" v-model:value="workday" :min="1" :max="31" clearable
              @update:value="radioChange(5)" @focus="radioChange(5)" />号最近的那个工作日
          </n-flex>
        </n-gi>
        <n-gi>
...
import { NGi, NCard, NGrid, NTabs, NTabPane, NTag, NRadioGroup, NSpace, NCheckbox, NCheckboxGroup, NRadio, NFlex, NInput, NInputNumber } from "naive-ui";
