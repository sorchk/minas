      ...
      <n-grid x-gap="20" y-gap="20" :cols="1">
        ...
        <n-gi>
          <n-flex align="center">
            <n-radio :value="2" />
            周期从
            <n-input-number class="nin" v-model:value="cycle01" :min="0" :max="58" clearable
              @update:value="radioChange(2)" @focus="radioChange(2)" />
            到
            <n-input-number class="nin" v-model:value="cycle02" :min="cycle01 ? cycle01 + 1 : 1" clearable
              @update:value="radioChange(2)" @focus="radioChange(2)" />秒
          </n-flex>
        </n-gi>
        <n-gi>
          <n-flex align="center">
            <n-radio :value="3" />
            周期从
            <n-input-number class="nin" v-model:value="average01" :min="0" :max="58" clearable
              @update:value="radioChange(3)" @focus="radioChange(3)" />
            秒开始，每
            <n-input-number class="nin" v-model:value="average02" :min="1" :max="59 - average01 || 0" clearable
              @update:value="radioChange(3)" @focus="radioChange(3)" />秒执行一次
          </n-flex>
        </n-gi>
        <n-gi>
          <n-flex>
            ...
            <div style="flex: 1">
              <n-checkbox-group v-model:value="checkboxList" @update:value="radioChange(4)">
                <n-space>
                  <n-checkbox v-for="item in 60" :key="item" :value="item - 1" :label="zeroFill(item - 1)" />
                </n-space>
              </n-checkbox-group>
            </div>
          </n-flex>
...
<script lang="ts">
export default { name: "CronGenSecond" };
</script>
