<template>
  <div style="width: 100%;">
    ...
    <n-form-item v-for="(item, i) in state.values" :key="i" :label="'参数[' + i + ']：'">
      <n-grid :cols="24" :x-gap="5">
        <n-grid-item :span="6">
          <n-input size="small" v-model:value="item.name" placeholder="参数名称"></n-input>
        </n-grid-item>
        <n-grid-item :span="1" style="margin: 0 5px 0 5px;">
          值：
        </n-grid-item>
        <n-grid-item :span="15">
          <n-input size="small" v-model:value="item.value" placeholder="参数值"></n-input>
        </n-grid-item>
        <n-grid-item :span="1">
...
import { ParamValue } from "@/components/dagdesigner/interface";
...
const onPaste = async () => {
  try {
    ...
    if (data.indexOf(COPY_PREFIX) == 0) {
      ...
      state.values = new Array<ParamValue>();
      values.forEach(item => {
        const kv = item.split(":", 2);
        state.values.push({ name: kv[0], value: kv[1] });
      })
...
//定义方法
const onAdd = () => {
  const param = new ParamValue();
  state.values.push(param);
  emit("update:modelValue", state.values);
}
