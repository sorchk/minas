    ...
    <div class="form">
      ...
      <n-form :model="model" ref="form" :rules="rules" label-placement="top">
        <n-form-item path="loginName" :label="t('fields.login_name')">
          <n-input round v-model:value="model.account" :placeholder="t('fields.login_name')" clearable>
            <template #prefix>
              <n-icon>
                <person-outline />
              </n-icon>
        ...
        <n-button round block type="primary" :disabled="submiting" :loading="submiting" @click.prevent="submit">{{
...
import { deepClone, useIsMobile } from "@/utils";
...
const { submit, submiting } = useForm(form, () => {
  const params = deepClone(model);
  params.password = bcrypt.hashSync(params.password, 10); 
  return systemApi.init(params)
}, () => {
