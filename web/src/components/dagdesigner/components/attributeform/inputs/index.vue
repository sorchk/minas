<template>
  <div style="width: 100%;">
    ...
    <n-form-item v-for="(item, i) in state.values" :key="i" :label="'[' + i + ']：'">
      <n-grid :cols="24" :x-gap="5">
        <n-grid-item :span="22">
          <n-input size="small" v-model:value="state.values[i]" :placeholder="placeholder"></n-input>
        </n-grid-item>
        <n-grid-item :span="1">
...
const onCopy = async () => {
  try {
    if (state.values.length > 0) {
      const data = COPY_PREFIX + state.values.join(", ");
      toClipboard(data)
